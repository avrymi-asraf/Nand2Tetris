...
//init screenLastAdress = SCREEN + 8192
@8192
D = A
...
(CHANGE_COLOR)
    ...
    //check if kbd is pressed, if yes go to black
    @BLACK
    D;JNE
    //else, go to white
...
(INCREASE)
    ...
    //check if screenPtr >= Screen + screenSize
    @screenLastAdress
    D=D-M

    //if yes return to main loop
    @MAIN_LOOP
    D;JGE

    //else return to loop_black
    @CHANGE_COLOR
    0;JMP
